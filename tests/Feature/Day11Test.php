...
{
    /** @test */
    public function parser_prepares_data_correctly()
    {
        // given
        // short input

        // when
        $parsed = Parser::parse(static::SHORT_INPUT);

        // then
        $this->assertCount(10, $parsed);
        $this->assertCount(10, $parsed[0]);
        $this->assertEquals('L', $parsed[0][0]);
        $this->assertEquals('.', $parsed[0][7]);
        $this->assertEquals('.', $parsed[6][0]);
        $this->assertEquals('L', $parsed[9][9]);
    }

    /** @test */
    public function it_advances_one_round_correctly()
    {
        // given
        $solver = new Solver(Parser::parse(self::SHORT_INPUT));

        // when
        $solver->advanceRound();

        // then
        $this->assertEquals(self::SHORT_INPUT_STATE2, (string)$solver);
    }

    /** @test */
    public function it_reaches_stable_state_for_short_input()
    {
        // given
        $solver = new Solver(Parser::parse(self::SHORT_INPUT));
        ...
        // then
        $this->assertEquals(self::SHORT_INPUT_STATE_STABLE, (string)$solver);
        $this->assertEquals(37, $solver->countOccupiedSeats());
    }

    /** @test */
    public function it_reaches_stable_state_for_long_input()
    {
        // given
        $solver = new Solver(Parser::parse(self::LONG_INPUT));
...
#.#####.##';
    ...
    const SHORT_INPUT_STATE_STABLE = '#.#L.L#.##
#LLL#LL.L#
L.#.L..#..
#L##.##.L#
#.#L.LL.LL
...
#.#L#L#.##';
    ...
    const LONG_INPUT = 'LLLLL.L.LLLLLLL.LL.LLLLL.LLLLLLL..LLLLL.LLLLL.L..LLLLL..LLLLLLLL.LLLL.LLL.LLLLLLL.LLLLLLLLLL
