...
{
    const SHORT_PASSPORTS = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in";
    ...
    /** @test */
    public function it_validates_short_passport_list_correctly()
    {
        // given
        $strings = collect(explode(PHP_EOL . PHP_EOL, static::SHORT_PASSPORTS));

        // when
        $valid = $strings->filter(function ($string) {
            return (new Passport($string))->validate();
        })->count();
    ...
    /** @test */
    public function it_validates_long_passport_list_correctly()
    {
        // given
        $strings = collect(explode(PHP_EOL . PHP_EOL, static::LONG_PASSPORTS));
        ...
        // then
        $this->assertEquals(228, $valid);
    }

    /** @test */
    public function it_passes_deep_test_for_invalid_passports()
    {
        // given
        $strings = collect(explode(PHP_EOL . PHP_EOL, static::INVALID_PASSPORTS));

        foreach ($strings as $string) {
            // when
            $passport = new Passport($string);

            // then
            $this->assertFalse($passport->validateDeep());
        }
    }

    /** @test */
    public function it_passes_deep_test_for_valid_passports()
    {
        // given
        $strings = collect(explode(PHP_EOL . PHP_EOL, static::VALID_PASSPORTS));
        ...
        foreach ($strings as $string) {
            ...
            // then
            $this->assertTrue($passport->validateDeep());
        }
    }

    /** @test */
    public function deep_validate_batch_file()
    {
        // given
        $strings = collect(explode(PHP_EOL . PHP_EOL, static::LONG_PASSPORTS));
        ...
        // when
        $valid = $strings->filter(function ($string) {
            return (new Passport($string))->validateDeep();
        })->count();

        // then
        $this->assertEquals(175, $valid);
    }
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719";
    ...
    const LONG_PASSPORTS = "pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
